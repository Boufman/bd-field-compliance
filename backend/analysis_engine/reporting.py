...
def build_report(records: List[Dict[str, Any]], report_period_end: Optional[date] = None) -> Dict[str, Any]:
    ...
    def _wo_row(r: Dict) -> Dict:
        return {
            ...
            "time_submitted": f"{r.get('date_submitted') or ''} {r.get('time_submitted') or ''}".strip(),
    ...
    # Remedial = same filter as fortnight KPI box (week_records)
    # No keyword / parts filter — every job in the 14-day window
    remedial = []
