...
LOGO_BD_PATH = _app_dir() / "assets" / "bd_logo.png"
LOGO_CUSTOMER_PATH = _app_dir() / "assets" / "smhs_logo.png"
...
def build_pdf(analysis: Dict[str, Any], cpv: Dict[str, Any] | None = None, charts=None) -> bytes:
    ...
    kpis = [
        _kpi_box("Fortnight Work Orders<br/>Completed", str(summary.get("total_wo_week", 0)), styles),
    ...
    story += [
        ...
        Paragraph("Instances of work orders by service area as a percentage of year-to-date volume (Top 10).", styles["small"]),
        Spacer(1, 3 * mm),
    ]
    ...
    if dept:
        ...
        for name, info in list(dept.items())[:10]:
            rows.append([
    ...
    story += [
        ...
        Paragraph(
            "Remedial and corrective actions undertaken this fortnight",
            styles["small"],
        ),
        Spacer(1, 3 * mm),
    ]
    ...
    story += [
        Paragraph(f"<b>{len(remedial)}</b> corrective maintenance activities completed.", styles["body"]),
        Spacer(1, 3 * mm),
    ]
    if remedial:
        rows = [[Paragraph(x, styles["table_header"]) for x in (
            "WO #", "Asset", "Department", "Product type", "Completion Comments"
        )]]
        for record in remedial:
            commentary = (
                (str(record.get("commentary", "") or "") )
                .strip()
            )
            rows.append([
                Paragraph(str(record.get("work_order", "")), styles["table_cell"]),
                Paragraph(str(record.get("asset", "") or ""), styles["table_cell"]),
                Paragraph(str(record.get("department", "") or ""), styles["table_cell_left"]),
                Paragraph(str(record.get("product_type", "") or ""), styles["table_cell_left"]),
                Paragraph(commentary, styles["table_cell_left"]),
            ])
        table = Table(
            rows,
            colWidths=[18 * mm, 22 * mm, 32 * mm, 30 * mm, 63 * mm],
            repeatRows=1,
        )
        table.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), NAVY),
    ...
    story += [
        ...
        Paragraph(
            "Detailed log of corrective maintenance work orders listed in Section 3. "
    ...
    if remedial:
        rows = [[Paragraph(x, styles["table_header"]) for x in (
            "WO #", "Time Submitted", "Time of Attendance", "Time of Completion",
            "Urgency", "Classification", "Asset", "KPI",
        )]]
        for record in remedial:
            ...
            rows.append([
                ...
                Paragraph(str(detail.get("time_submitted") or ""), styles["table_cell"]),
                Paragraph(str(detail.get("time_of_attendance") or record.get("date") or ""), styles["table_cell"]),
                Paragraph(str(detail.get("time_of_completion") or record.get("date") or ""), styles["table_cell"]),
                Paragraph(str(detail.get("urgency") or ""), styles["table_cell"]),
                Paragraph(str(detail.get("hw_sw") or ""), styles["table_cell"]),
                Paragraph(str(record.get("asset") or detail.get("asset_number") or ""), styles["table_cell"]),
                kpi_cell,
            ])

        table = Table(
            rows,
            colWidths=[16*mm, 28*mm, 28*mm, 28 *mm, 15*mm, 18*mm, 22*mm, 12*mm],
            repeatRows=1,
        )
